 ...
 * git du prof : https://github.com/ctiber/composants
 */
@Stateful
public class ConverterBean implements IConverter {

	public static List<Monnaie> monnaies = new ArrayList<>();
    /**
     * Default constructor. 
     */
	...
	public  List<Monnaie> getAvailableCurrencies() throws IOException, JDOMException {
        ...
        // for First URL
        Element firstCube = racine_1.getChild("Cube");
        System.err.println(firstCube);
        Element secondeCube = firstCube.getChild("Cube");
        List<Element> othersCurrency = secondeCube.getChildren();
        
        //for Second URL
        List<Element> otherCountries = racine_2.getChild("CcyTbl").getChildren();
        ...
        for(Element currency : othersCurrency) {
        	...
        	for(Element country : otherCountries) {
        		if(country.getChild("Ccy").getValue().equals(m.getCurrencyCode())) {
        			m.setCountryName(country.getChild("CtryNm").getValue());
        			m.setCurrencyCode(country.getChild("CcyNm").getValue());
        		}
        	}
