...
<form name="Changez votre monnaie" action="index.jsp" method="get">
	<label>Veuillez saisir le montant à convertir : </label>
	...
	<label>Veuillez selectionner la monnaie cible: </label>
	
	<br/>
	<select name="monnaie">  
	<%
		for(Monnaie m : beanConv.getAvailableCurrencies()){
			out.println("<option value=" + m.getRate() + ">"+ m.getCurrencyCode() + "</option>");
		}
	%> 
	<!--  
	...
	<br/> 
	<input type="email" id="email" pattern=".+@.+" size="50">
...
<%
		...
		if(email != null && email.length() != 0) {
			...
			out.println("<h4> Le montant saisi est : </h4>" + amount);
			
			amount = beanConv.euroToOtherCurrency(amount, Double.parseDouble(request.getParameter("monnaie")));
